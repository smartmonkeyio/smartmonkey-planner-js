...
/**
 * Returns the Joi validation to validate a time window
 */
...
/**
 * Returns the Joi validation for capacity/volume of vehicles and stops
 */
    ...
    .items(
      joi.object({
        ...
        duration: joi
          ...
          .min(0)
          .max(24 * 3600),
        time_windows: joi.array().items(timeWindow()).single(),
        size: capacity(),
      })
    )
    .single();
